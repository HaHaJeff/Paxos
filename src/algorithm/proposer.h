...
class ProposerState {
  public:
    ProposerState(StateMachine *pState) : maxRound_(0), index_(0), prepared_(false), pState_(pState) {  }
    ~ProposerState() {  }
...
//    void SetFirstUnchosenIndex(uint32_t index) { firstUnchosenIndex_ = index; }
    ...
    void AddToStateMachine(const ProposalEntry &entry);
...
//   uint32_t nextIndex_;
    ...
    StateMachine *pState_;
};

class Proposer {
  public:
    Proposer(StateMachine *pState);
    ~Proposer();
